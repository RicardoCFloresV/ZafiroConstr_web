...
const PROTECTED_UNITS = ['Sin Unidad'];
...
Router.post('/update', requireAuth, async (req, res) => {
  try {
    ...
    const existing = await db.executeProc('units_get_by_id', { unit_id: { type: sql.Int, value: Number(body.unit_id) } });
    if (existing?.length && PROTECTED_UNITS.includes(existing[0].nombre)) {
